...
and contains data about daily vaccinations divided by region, age range and vaccine supplier. \\ 
...
\begin{itemize}
  \item[] We assumed that the general user doesn't know the italian language, therefore
    ...
    english translation. For the file dpc-covid19-ita-regioni.csv we manteined the italian
    language for all the fields' names due to a problem with the ingestion. Only the common
    field with the other files has been renamed.
\end{itemize}
    ...
    \begin{itemize}
      ...
      \item ISTAT\_region\_code (codice\_regione\_ISTAT): \emph{keyword} \\
        \begin{footnotesize}
          ...
          enumerated and aggregation is possible. Additionaly, in order to fully utilize
    ...
    \begin{itemize}
      ...
      \item ISTAT\_region\_code: \emph{keyword} \\
        \begin{footnotesize}
          ...
          be useless to execute arithmetical operations on this value due to its nature.
        \end{footnotesize}
      \item deceduti: \emph{long} \\
        \begin{footnotesize}
          Total number of people that are deceased due to Covid19, mapped as long since it 
          represents a large number.
        \end{footnotesize}
      \item dimessi\_guariti: \emph{long} \\
        \begin{footnotesize}
          Total number of people that have been discharged from the hospital because they have 
          recovered from Covid19, mapped as long since it represents a large number.
        \end{footnotesize}
      \item isolamento\_domiciliare: \emph{long} \\
        \begin{footnotesize}
          Total number of people that are home isolated due to Covid19, mapped as long since it 
          represents a large number.
        \end{footnotesize}
      \item lat: \emph{double} \\
        \begin{footnotesize}
          Latitude of the region, mapped as double since it's a float number.
        \end{footnotesize}
      \item long: \emph{double} \\
        \begin{footnotesize}
          Longitude of the region, mapped as double since it's a float number.
        \end{footnotesize}
      \item note: \emph{text} \\
        \begin{footnotesize}
          General notes from the government, mapped as text in order to be better suited for queries
          that utilize an analyzer.
        \end{footnotesize}
      \item note\_casi: \emph{text} \\
        \begin{footnotesize}
          Notes related to the tested cases from the government, mapped as text in order to be better 
          suited for queries that utilize an analyzer.
        \end{footnotesize}
      \item nuovi\_positivi: \emph{long} \\
        \begin{footnotesize}
          Daily new number of people that have been tested positive due to Covid19, mapped as long since it 
          represents a large number.
        \end{footnotesize}
      \item ricoverati\_con\_sintomi: \emph{long} \\
        \begin{footnotesize}
          Total number of people that have been hospitalized with symptoms to Covid19, mapped as long 
          since it represents a large number.
        \end{footnotesize}
      \item stato: \emph{keyword} \\
        \begin{footnotesize}
          Code indicating the country, mapped as keyword since entries are 
          enumerated and aggregation is possible.
        \end{footnotesize}
      \item tamponi: \emph{long} \\
        \begin{footnotesize}
          Total number of people that have been tested to Covid19, mapped as long 
          since it represents a large number.
        \end{footnotesize}
      \item terapia\_intensiva: \emph{long} \\
        \begin{footnotesize}
          Total number of people that are in intensive care due to Covid19, mapped as long 
          since it represents a large number.
        \end{footnotesize}
      \item totale\_casi: \emph{long} \\
        \begin{footnotesize}
          Total number of people that have been tested to Covid19, mapped as long 
          since it represents a large number.
        \end{footnotesize}
      \item totale\_ospedalizzati: \emph{long} \\
        \begin{footnotesize}
          Total number of people that have been hospitalized due to Covid19, mapped as long 
          since it represents a large number.
        \end{footnotesize}
      \item totale\_positivi: \emph{long} \\
        \begin{footnotesize}
          Total number of people that have been tested positive to Covid19, mapped as long 
          since it represents a large number.
        \end{footnotesize} 
      \item variazione\_totale\_positivi: \emph{long} \\
        \begin{footnotesize}
          Daily variation of the total number of people that have been tested positive to Covid19, 
          mapped as long since it represents a large number.
        \end{footnotesize}
      \item location: \emph{geo_point} \\
        \begin{footnotesize}
          The combination of the lat and long fields, mapped as geo_point since it
...
For each day, region, age range and vaccine supplier we have information about: the number of vaccinated people (male and female), the number of 1st, 2nd and booster doses and information 
...
In addition, we used the ‘dpc-covid19-ita-regioni.csv’ dataset to have more information about
the pandemic, from the total number of positive to the number of hospitalized and deceased people.
...
Total number of people who was infected by Covid-19 within 3 to 6 months before the date of the vaccination in each region.
\begin{tcolorbox}[fontupper=\scriptsize]
  \begin{verbatim}
GET /index-vaccines_administrations/_search
{
...
It return the percentage of fully vaccinated people (i.e. having 1st and 2nd dose) in each region.\\
To do so, we used data about the total population of each region contained into \emph{platea.csv}.
\begin{tcolorbox}[fontupper=\scriptsize]
  \begin{verbatim}
...
It returns the number of healed people over the total population for each region region. \\
We used:
 \begin{itemize}
\item The data about the total population in each region from \emph{platea.csv};
\item The number of healed people for each region that can be found in \emph{soggetti-guariti.csv}.
 \end{itemize}
\begin{tcolorbox}[fontupper=\scriptsize]
  \begin{verbatim}
...
It returns, for each day, the total number of cases, hopitalizations and the total number of positives. \\
We used data coming from \emph{dpc-covid-19-ita-regioni.csv}.
\begin{tcolorbox}[fontupper=\scriptsize]
  \begin{verbatim}
...
It returns the ratio between positive and hospitalized people in each region.
 \todo{ISTAT code da che index arriva?}
\begin{tcolorbox}[fontupper=\scriptsize]
  \begin{verbatim}
GET /index-vaccines_administrations/_search 
{
...
It represents the percentage of people with at least one, two or three doses together with the 
